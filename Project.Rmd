...
## 4. How much should the salary increase be when you get promoted to a higher experience_level?
...
f + ggtitle("Clusters of salary for entry level") + xlab("") + ylab("Salary (USD)")
...
f <- ggplot(data=dfMI,mapping=aes(x='MI',y=salary_in_usd))+
  geom_violin(scale="count")+
  geom_point(aes(colour=cluster))
f + ggtitle("Clusters of salary for mid level") + xlab("") + ylab("Salary (USD)")
...
f <- ggplot(data=dfSE,mapping=aes(x='SE',y=salary_in_usd))+
  geom_violin(scale="count")+
  geom_point(aes(colour=cluster))
f+ ggtitle("Clusters of salary for senior level") + xlab("") + ylab("Salary (USD)")
...
f <- ggplot(data=dfEX,mapping=aes(x='EX',y=salary_in_usd))+
  geom_violin(scale="count")+
  geom_point(aes(colour=cluster))
f+ ggtitle("Clusters of salary for executive level") + xlab("") + ylab("Salary (USD)")
```
...
executive_fair_mean = mean(dfEX$salary_in_usd[dfEX$cluster==2])
...
senior_to_executive_increase_absolute  = executive_fair_mean - senior_fair_mean

entry_to_middle_increase_absolute
middle_to_senior_increase_absolute
senior_to_executive_increase_absolute
```
...
senior_to_executive_increase_relative = (((senior_fair_mean+senior_to_executive_increase_absolute)/senior_fair_mean)-1)

cat(entry_to_middle_increase_relative*100,"%\n")
cat(middle_to_senior_increase_relative*100,"%\n")
cat(senior_to_executive_increase_relative*100, "%\n")
```
...
```{r}
df_filtered <- filter(df, employee_region == company_region & remote_ratio == "Office")

print(ggplot(aes(x = employee_region, y = salary_in_usd), data = df_filtered) +
    stat_summary(fun=mean, geom="bar", position = "stack") +
    xlab("Region") + labs(title="Salary of Office work by Region"))
```
...
print(ggplot(aes(x = company_region, y = salary_in_usd), data = df_filtered) +
    stat_summary(fun=mean, geom="bar", position = "stack") +
    xlab("Region") + labs(title="Salary of Remote work by Region"))
```
...
for (i in 1:nrow(aux)) {
  max = 0
  for (j in 1:ncol(aux)) {
    df_filtered <- filter(df, company_region == colnames(aux)[j] & employee_region == rownames(aux)[i] & remote_ratio == "Remote")
