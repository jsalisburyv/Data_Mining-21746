...
```
```{r}
#1: monica. 2: dawid. 3: TODO wallis
datos <- Tree_test[1,]
...
grid <- expand.grid(
  ...
  employment_type = levels(datos$employment_type),
  job_title = c("ML Engineer"),
  salary_in_usd = 0,
  employee_residence = levels(datos$employee_residence),
  remote_ratio = levels(datos$remote_ratio),
  company_location = c("US"),
  company_size = levels(datos$company_size),
  employee_region = c("North America"),
  company_region = c("North America"),
...
```{r}
prediction <- vector("numeric", 6812)
for(i in 1:nrow(grid)){
  prediction[i] <- rpart.predict(fit, newdata = grid[i,])
}
```

```{r}
best_combination <- grid[which.max(prediction), ]
best_combination
```
...
What factors seem to have the most impact on determining an employee's job level within the company, as indicated by the data provided in the dataset?

```{r}
set.seed(123)
df_no_s <- select(df, -salary_group, -salary_currency, -salary_in_usd)
...
confusionMatrix(predictions, test_data$experience_level)
confusionMatrix$byClass[1]
```
```
