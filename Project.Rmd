...
```{r}
dfAux = df[c('salary_in_usd','experience_level')]
levels(dfAux$experience_level) <- c(1,4,2,3)
cls <- kmeans(x = dfAux, centers = 3)
dfAux$cluster <- as.character(cls$cluster)
f <- ggplot()+
  geom_violin(data=dfAux,scale="area",mapping=aes(x = reorder(experience_level,salary_in_usd), y = salary_in_usd))+
  geom_point(data=dfAux, mapping= aes(x = reorder(experience_level,salary_in_usd), y = salary_in_usd, colour = cluster))
f
```
...
#dfEN <- dfEN[dfEN.loc(dfEN$salary_in_usd>quantiles[1] & dfEN$salary_in_usd < quantiles[2],)]
#TODO: FIX
